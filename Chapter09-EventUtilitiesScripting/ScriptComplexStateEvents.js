...
// Reference:   http://X3dGraphics.com/examples/X3dForWebAuthors/Chapter09-EventUtilitiesScripting/ScriptComplexStateEvents.x3d
...
function buttonMotionDone (eventValue) 
{
    if (eventValue == false) // eventValue is false when button touch is released
    {
        ...
        if (buttonPushCount == 0) 
        {
            ...
            lightBulbColor = new SFColor (.2, .2, .2); // off
        } 
        else if (buttonPushCount == 1) 
        {
            lightBulbColor = new SFColor (.6, .5, .2); // low
        } 
        else if (buttonPushCount == 2) 
        {
            lightBulbColor = new SFColor (.7, .7, .2); // medium
        } 
        else  if (buttonPushCount == 3) 
        {
            lightBulbColor = new SFColor (1, 1, .6);   // high
        }
